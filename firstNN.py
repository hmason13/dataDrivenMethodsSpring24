...
train_data = data_ANN_normalize(dataVel,dataEta,device = "cuda")
val_data = data_ANN_normalize(testVel,testEta,device = "cuda")
...
model = ANN(num_in=5000, num_out=2500, num_layers=6, num_hidden=6).to(device = "cuda")
...
train(model, train_loader, test_loader, 100, loss, optimizer)
...
test_data = data_ANN_normalize(exmpVel,exmpEta,device = "cuda")
...
model = model.eval()
with torch.no_grad():
    pred = np.reshape(model(test_data[100,:]), (50,50))

plt.imshow(pred, cmap="seismic", vmax=0.5, vmin=-0.5)
plt.savefig("/scratch/hcm7920/class/plots/nnOutput.png")
plt.imshow(exmpEta, cmap="seismic", vmax=0.5, vmin=-0.5)
plt.savefig("/scratch/hcm7920/class/plots/myTruth.png")
